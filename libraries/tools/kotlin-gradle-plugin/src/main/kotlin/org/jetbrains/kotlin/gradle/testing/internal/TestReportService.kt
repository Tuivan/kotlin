...
abstract class TestReportService : BuildService<BuildServiceParameters.None> {
    ...
    fun reportFailure(taskPath: String, originTaskPath: String, failure: Error) {
        taskSuppressedFailures.computeIfAbsent(taskPath) { mutableListOf() }.add(originTaskPath to failure)
    }
