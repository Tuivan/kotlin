...
open class KotlinTestReport : TestReport() {
    ...
    private val failedTestsListener = FailedTestListener(parentPaths, path, testReportServiceProvider)
    ...
    private class SuppressedTestRunningFailureListener(
        private val allParents: Provider<List<String>>,
        ...
        override fun runningFailure(failure: Error) {
            allParents.get().forEach {
                testReportServiceProvider.get().reportFailure(taskPath, it, failure)
            }
            testReportServiceProvider.get().reportFailure(taskPath, taskPath, failure)
        }
    }
    ...
    private class FailedTestListener(
        ...
        private fun reportFailure(result: TestResult) {
            if (result.failedTestCount > 0) {
                ...
                testReportServiceProvider.get().testFailed(taskPath)
            }
        }
    }
}
