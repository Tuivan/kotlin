...
fun Project.getTarget(): Named? = project.extensions.findByName("kotlin")?.let { kotlinExt ->
    require(kotlinExt.javaClass.name in kotlinSingleTargetExtensionClassNames) {
        "`getTarget` call is unavailable for ${kotlinExt.javaClass.name} extension class"
    }
    kotlinExt["getTarget"] as? Named
}
